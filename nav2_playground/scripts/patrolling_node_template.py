...
"""Patrolling node for Nav2 - Template for students."""
        ...
        if self.state == PatrolState.IDLE:
            ...
            if status == GoalStatus.STATUS_ACCEPTED:
                ...
                self.get_logger().info(
                    ...
                    f'{len(self.waypoints)}')
                self.current_goal_handle = None
                self.state = PatrolState.DO_SOMETHING_AT_WAYPOINT

            elif status == GoalStatus.STATUS_ABORTED:
                self.get_logger().error('Navigation was aborted by the action server')
                self.state = PatrolState.ERROR
